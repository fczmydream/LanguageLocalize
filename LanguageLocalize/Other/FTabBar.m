...
- (void)layoutSubviews
{
    ...
    self.plusBtn.centerX = self.centerX - 3*FMagin;
    //调整发布按钮的中线点Y值
    self.plusBtn.centerY = self.height * 0.5 - 5*FMagin;
    ...
    label.text = Local_c(@"Post");
    label.font = [UIFont systemFontOfSize:11];
    [label sizeToFit];
    label.textColor = [UIColor grayColor];
    [self addSubview:label];
    label.centerX = self.plusBtn.centerX;
    label.centerY = CGRectGetMaxY(self.plusBtn.frame) + FMagin ;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if(self.isHidden == NO)
    {
        ...
        CGPoint pointA = [self convertPoint:point toView:self.plusBtn];
        //判断如果这个新的点是在发布按钮身上，那么处理点击事件最合适的view就是发布按钮
        if([self.plusBtn pointInside:pointA withEvent:event])
        {
            return self.plusBtn;
        }
